...
export function ProjectsPage({ projects, companies, subcontractors, onRefresh }: any) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    await window.dobiApi.createProject({ ...form, linked_company_id: form.linked_company_id || undefined });
    ...
    <div className="module-stack">
      <section className="panel">
        ...
        <form className="inline-form" onSubmit={submit}>
          ...
          <label>Firma</label><select value={form.linked_company_id} onChange={(e)=>setForm({...form,linked_company_id:Number(e.target.value)})}><option value={0}>Seçiniz</option>{companies.map((c:any)=><option key={c.id} value={c.id}>{c.name}</option>)}</select>
