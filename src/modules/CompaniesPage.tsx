...
export function CompaniesPage({ rows, movements, onRefresh }: { rows: any[]; movements: any[]; onRefresh: () => Promise<void> }) {
  ...
  const filtered = useMemo(
    () => rows
      ...
      .sort((a, b) => (sort === 'name' ? a.name.localeCompare(b.name) : b.current_balance - a.current_balance)),
    [rows, filter, sort]
  );

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await window.dobiApi.createCompany(form);
    if (!res.ok) {
      setMessage('Firma kaydedilemedi. Kod benzersiz olmalı.');
      return;
    }
